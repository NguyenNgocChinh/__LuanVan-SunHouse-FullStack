<x-jet-action-section>
    ...
    <x-slot name="content">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('Thiết bị đang sử dụng') }}</span>
                                    @else
                                        {{ __('Đăng nhập lần cuối') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            <x-slot name="title">
                {{ __('Đăng xuất các phiên trên trình duyệt khác') }}
            </x-slot>
